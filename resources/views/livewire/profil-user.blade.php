        ...
        <div class="absolute right-0 mt-2 px-2   bg-white shadow-lg rounded-lg  z-50">
            <ul class="divide-y divide-gray-200 w-64 max-h-[80vh] min-h-[10vh] overflow-auto">
                @forelse($user->unreadNotifications as $notif)
                    <li class="px-4 py-2 text-sm hover:bg-gray-100 flex justify-between cursor-pointer"  wire:click="update('{{ $notif->id }}')"
                        >
                        <span>
                            {{ $notif->data['pesan']  }}
                        </span>
            ...
            <div class="flex justify-center p-2">
              <button wire:click="ClearAll" class="border-slate-400 border hover:bg-slate-300 active:bg- rounded px-2 py-1">Tandai Semua</button>
            </div>
        </div>
    @endif
</div>
...
<script>
    var userId = "{{ auth()->id() }}"; // ambil ID user login
